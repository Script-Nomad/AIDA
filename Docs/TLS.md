...
AIDA picks a TLS strategy from the flag you pass to `./start.sh`. There is no
...
```bash
./start.sh --domain aida.example.com
...
`.aida/Caddyfile` (gitignored). This file is regenerated on every run from
the flags you pass — don't edit it directly.

`.aida/Caddyfile` is consumed by Caddy via the `docker-compose.tls.yml`
...
Postgres data is in the `aida_postgres_data` Docker volume and is **never**
